...
//Для тестов различных элементов интерфейса
...
import static com.codeborne.selenide.Selenide.*;
    ...
    public class Snippets {
        ...
        void browser_command_examples() {
            ...
            var cookie=new Cookie("foo","bar"); // создаем объект Cookie с именем cookie. В нем первый параметр - имя записи в Cookies, второе значение
            WebDriverRunner.getWebDriver().manage().addCookie(cookie); // Дбавляем куки в браузер
        }
        ...
        void selectors_examples() {
            ...
            $("div", 2).click(); // find the third div

            $x("//h1/div").click();
            $(byXpath("//h1/div")).click();

            $(byText("full text")).click(); // ищет полностью текст элемента
            $(withText("ull tex")).click(); // ищет текс по подстроке

            $(byTagAndText("div","full text")); // поиск текста в паре с тегом
            $(withTagAndText("div","ull text"));// поиск подстроки в паре с тегом

            //Перемещение по древу DOM
            $("").parent();               // перемещение по древу на 1 элемент вверх
            $("").sibling(1);       // ищет сиблингов вниз (элементы того же уровня внутри элемента)
            $("").preceding(1);     // ищет сиблингов вверх
            $("").closest("div");  // ищет ближайший элемент-родитель (параметры тег|класс|параметр)
        ...
        void actions_examples() {
            ...
            $("").setValue("text"); // стирает все из поля input и добавляет туда "text"
            $("").append("text");   // добавляет в конец к уже введенному в input
            $("").clear();
            $("").setValue(""); // clear
